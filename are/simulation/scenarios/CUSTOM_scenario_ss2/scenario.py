...
class ss2(Scenario):
    ...
    def build_events_flow(self) -> None:
        ...
        with EventRegisterer.capture_mode():
            event1 = agui.send_message_to_agent(
                content="Please update the phone number of Jane Doe to 1234567890",
            ).depends_on(None, delay_seconds=2)

            oracle1 = (
                db.update_db_entry(self.target_id, phone="1234567890")
                .oracle()
    ...
    def validate(self, env) -> ScenarioValidationResult:
        ...
        # try:
        #     target_event = next(event for event in agent_events if event.action.function_name == "get_db_entry" and event.action.args["id"] == self.target_id)
